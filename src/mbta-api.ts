...
export async function fetchAndCacheAllStops(): Promise<Stop[]> {
    const [lightStops, heaveyStops] = await Promise.all([
        fetchStopsByRouteType(RouteType.LIGHT_RAIL),
        fetchStopsByRouteType(RouteType.HEAVY_RAIL)
    ]);
 ...
 */
async function fetchStopsByRouteType(routeType: RouteType): Promise<Stop[]> {
    console.log(`Fetching stops for route type ${routeType}`);
